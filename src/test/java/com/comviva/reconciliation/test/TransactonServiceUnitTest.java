...
public class TransactonServiceUnitTest {
	...
	final static StatusCheckResponse failedStatus = new StatusCheckResponse("EXRCSTATRESP", "500", "01/08/2016 15:49:24", "01557919",
			"V160729.1550.110001", "200",
	...
	final static StatusCheckResponse successStatus = new StatusCheckResponse("EXRCSTATRESP", "200", "01/08/2016 15:49:24",
			"01557919", "V160729.1550.110001", "200",
			"Last transfer status of transaction ID:V160729.1550.110001, transfer date "
...
/*	@Test
	public void processFailedTransaction_SendStatusCheck_Failure_Test(){
		Mockito.when(transactionDao.getFailedTransactions()).thenReturn(failedTransactions);
		Mockito.when(requestService.sendStatusCheckRequest(ArgumentMatchers.any(Transaction.class))).thenReturn(failedStatus);		
		...
		for(Transaction trans: failedTransactions) {
			...
			verify(requestService, never()).sendReversalRequest(trans);
			verify(transactionDao, times(1)).updateTransaction(trans);
		}
	
	}*/
	
	@Test
	public void processFailedTransaction_SendStatusCheck_Success_Reversal_Failure_Test(){
		Mockito.when(transactionDao.getFailedTransactions()).thenReturn(failedTransactions);
		Mockito.when(requestService.sendStatusCheckRequest(ArgumentMatchers.any(Transaction.class))).thenReturn(successStatus);
		Mockito.when(requestService.sendReversalRequest(ArgumentMatchers.any(Transaction.class))).thenReturn(false);
		
		transactionService.processFailedTransaction();
		
		for(Transaction trans: failedTransactions) {
			verify(requestService, times(4)).sendStatusCheckRequest(trans);
			verify(requestService, times(28)).sendReversalRequest(trans);
